...
router.get('/home',middleware.isAllowed,function(req, res, next) {
  Post.find((err,post)=>{
  	  if(err) next(err);
  	  res.render('home',{title:"Steel Smiling",header:true,navbar:true,user:req.user,isImage:req.user.image.includes("http"),posts:post,single:false});
  }).sort({'date': -1});
});
...
router.get('/profile/:id',middleware.isAllowed,function(req,res,next){
	db.findData(User,{_id:req.params.id})
	  .then(function(data){
	  	  res.render('profile',{title:"Steel Smiling",header:true,navbar:true,user:data,allow: (req.user._id == data._id),isFollow:data.followers.find(user => user == req.user._id )});
	  }).catch(function(err){
	  	next(err);
	  });
});
...
router.get('/update',middleware.isAllowed,function(req,res,next){
	...
	db.findData(User,{_id:req.user._id})
	  .then(function(data){
	  	 res.render('settings',{title:"Steel Smiling",header:true,navbar:true,user:data,isImage:req.user.image.includes("http")});
	  }).catch(function(err){
	  	next(err);
	  });
...
router.get('/refer',middleware.isAllowed,function(req,res,next){
    db.findData(User,{_id:req.user._id})
        .then(function(data){
            res.render('referral',{title:"Steel Smiling",header:true,navbar:true,user:data});
        }).catch(function(err){
        next(err);
    });
