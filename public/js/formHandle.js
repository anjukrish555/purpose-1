...
const registermhp = document.querySelector('#registerMHP');
    ...
    $.ajax({
        ...
        success : function(response){
            ...
            let data = users.map(user => {
                ...
                html +=  `<span>${user.username}, ${user.role}</span>
...
function validateFile(){
    ...
    var types = ['image/jpeg','image/png','video/mp4'];
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.success) {
                ...
                html += `<span class="time" style="">${post.todate}</span> </div>
                    </div>
...
if(registermhp) registermhp.addEventListener('submit',register_mhp);
if(signup) signup.addEventListener('submit',register_user);
if(filterResources) filterResources.addEventListener('submit',filter_resources);
    ...
    $.ajax({
        ...
        success : function(response){
            ...
            if(!response.success){
                if(response.msg){
                    flashModal = document.querySelector('#flashModal .modal-body');
                    flashModal.innerHTML = `<center id="error" class="flash">
                        			 		 <center> <i class="fa fa-times-circle"> </i></center>
                          			  		<p class="lead" style="color:#a9a9a9">${response.msg}</p>
                      					</div>`;
                    $('#flashModal').modal('show');
                }else{
    ...
    sendBtn.addEventListener('click',function(){
        ...
        $.ajax({
            ...
            error  : function(response){
                flashModalChat.innerHTML = `<div id="error" class="flash">
                        			 		 <i class="fa fa-times-circle"></i>
                          			  		<p class="lead" style="color:#a9a9a9">Something went wrong here.</p>
                      					</div>`;
